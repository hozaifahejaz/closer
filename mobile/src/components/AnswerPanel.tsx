...
export function AnswerPanel({ room, cardKey, partnerName, onLockIn, onFocus }:
  { room: RoomState; cardKey: string; partnerName: string; onLockIn: (text: string) => boolean; onFocus?: () => void }) {
      ...
      {room.revealed ? (
        <View style={{ gap: 10 }}>
          {room.revealed.map((a, i) => (
            <Animated.View key={i} style={[st.reveal, { opacity: pop, transform: [{ translateY: pop.interpolate({ inputRange: [0, 1], outputRange: [8 + i * 6, 0] }) }, { scale: pop.interpolate({ inputRange: [0, 1], outputRange: [0.97, 1] }) }] }]}>
              <Text style={st.revealWho}>{a.mine ? 'You' : a.name}</Text>
              <Text style={st.revealText}>{a.text}</Text>
            </Animated.View>
          ))}
        </View>
      ) : answered ? (
        <View>
          ...
          <Text style={st.mine}>{room.myAnswer}</Text>
        </View>
      ) : (
        <View style={{ gap: 10 }}>
          <TextInput value={text} onChangeText={setText} onFocus={onFocus} multiline maxLength={1000} placeholder="Write your answer…"
          ...
          <Button title="Lock in my answer" icon="lock" disabled={!text.trim()}
            onPress={() => { if (onLockIn(text.trim())) setText(''); }} />
        </View>
      )}
...
const st = StyleSheet.create({
  box: { backgroundColor: colors.bg2, borderWidth: 1, borderColor: colors.line, borderRadius: radius.box, padding: 14 },
