...
export function DeckPicker({ room, send }: { room: RoomState; send: (a: any) => void }) {
  ...
  const cols = width >= 900 ? 3 : width >= 340 ? 2 : 1;
  ...
  const tile = (name: string | null, label: string, blurb: string, icon: IconName, cards: number, fresh_: number, on: boolean) => {
    ...
    return (
      <Pressable key={label} onPress={() => toggle(name)} accessibilityRole="checkbox" accessibilityState={{ checked: on }}
        ...
        style={({ pressed }) => [st.tile, { width: `${100 / cols}%` as any }, pressed && { transform: [{ scale: 0.98 }] }]}>
        <View style={[st.tileIn, on && st.tileOn, spent && { opacity: 0.55 }]}>
          ...
          <Text style={st.nm}>{label}</Text>
          <Text style={st.bl}>{blurb}</Text>
  ...
  return (
    <Modal visible={room.choosing} animationType="slide" presentationStyle={Platform.OS === 'ios' ? 'pageSheet' : 'fullScreen'}
      ...
      <View style={[st.sheet, { paddingTop: Platform.OS === 'ios' ? 12 : insets.top + 8 }]}>
        ...
        <View style={st.head}>
          <View style={{ flex: 1 }}>
            <Text style={st.title} accessibilityRole="header">Choose your decks</Text>
            <Text style={st.sub}>Pick one, a few, or all of them. Your place in each is saved.</Text>
          </View>
          {room.started ? (
        ...
        <View style={[st.foot, { paddingBottom: Math.max(insets.bottom, 16) }]}>
          {room.couple ? (
            <View style={st.opt}>
              <View style={{ flex: 1 }}>
          ...
          <Button title={go} onPress={start} disabled={!picked.length || (fresh && !unused)} />
        </View>
      </View>
    </Modal>
  );
...
const st = StyleSheet.create({
  ...
  grid: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 14, paddingBottom: 12, width: '100%', maxWidth: 980, alignSelf: 'center' },
  tile: { padding: 6 },
  tileIn: { flex: 1, minHeight: 150, borderRadius: radius.box, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.bg2, padding: 14, overflow: 'hidden' },
  ...
  bl: { marginTop: 4, fontFamily: fonts.sans, fontSize: 13, lineHeight: 18, color: colors.muted, flexGrow: 1 },
  ct: { marginTop: 10, fontFamily: fonts.sansBold, fontSize: 12, color: colors.faint, letterSpacing: 0.3 },
